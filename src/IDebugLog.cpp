...
IDebugLog::IDebugLog(const char *name) { Open(name); }
...
void IDebugLog::OpenRelative(int folderID, const char *relPath) {
  ...
  if (!SUCCEEDED(err)) {
    _FATALERROR("SHGetFolderPath %08X failed (result = %08X lasterr = %08X)",
 ...
 *	@param source the source of the message, or NULL to use the previous
 *source
 */
 ...
 */
void IDebugLog::ClearSource(void) { sourceBuf[0] = 0; }

/**
 *	Increase the indentation level
 */
void IDebugLog::Indent(void) { indentLevel++; }
 ...
 */
void IDebugLog::CloseBlock(void) { inBlock = 0; }
 ...
 */
void IDebugLog::SetAutoFlush(bool inAutoFlush) { autoFlush = inAutoFlush; }
 ...
 *
 *	@note The cursor move will not be performed if the request would move
...
/**
 *	Returns the number of spaces a tab would occupy at the current cursor
 *position
 */
int IDebugLog::TabSize(void) { return ((~cursorPos) & 3) + 1; }

/**
 *	Rounds a number of spaces to the nearest tab
 */
int IDebugLog::RoundToTab(int spaces) { return (spaces + 3) & ~3; }

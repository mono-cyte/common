...
ISegmentStream::ISegmentStream() { streamLength = 0; }
...
void ISegmentStream::AddSegment(UInt64 offset, UInt64 length,
                                UInt64 parentOffset) {
...
ISegmentStream::SegmentInfo *ISegmentStream::LookupInfo(UInt64 offset) {
  for (SegmentInfoListType::iterator iter = segmentInfo.begin();
       iter != segmentInfo.end(); iter++)
    if ((offset >= (*iter).offset) &&
        (offset < (*iter).offset + (*iter).length))
      return &(*iter);
